...
- curl、jq、yq
- unzip、tar、7z（p7zip）、gzip
- sha256sum（通常随 coreutils 提供，install 会检测）
...
scripts:
  ...
  # package 流程总超时（秒，默认 60；generic 下载与 docker pull 均适用）
...
**config_hash**（可选，所有 service 类型）：

- 对 easy-deploy-config.yaml 中该 service 的 **package + deploy** 段做稳定 hash（yq 序列化 → gzip → sha256）
- worker 启动时与 json 中记录的 `config_hash` 比对；**缺失或不一致**时强制走 package + deploy（即使 version/digest 未变）
- worker 计算当前配置 hash 后会立即回存 `config_hash`，并在 **任意退出**时用 EXIT trap 再兜底回存一次（含 deploy 失败；失败后下次 hash 已对齐，package 仍可按 blocked 正常 skip）
- 升级后旧 json 无 `config_hash` 时，首次运行会对各 service force redeploy 一次以写入初始 hash
...
- `scripts.package-timeout-seconds` 若配置则须为正整数
...
- 依赖命令存在：curl、jq、yq、docker、`docker compose`、unzip、tar、7z、gzip、sha256sum
...
1. `on-agent-start`
2. `versions_ensure` 合并 `current-versions.json`
3. 读取 `./easy-deploy-config.yaml` 里所有 `services` 的 `name`，并行启动 `easy-deploy-worker.sh`
4. **等待全部 worker** 结束（失败计入 failures）
5. 清空 `./data/temp/`
6. 全部 worker 成功 → `on-agent-success`；存在失败 → `on-agent-fail`（`hook_fail_count`）
7. 日志保留策略，释放 easy-deploy.lock

**某个 worker 失败不影响其他 worker**，各 service 完全独立并行；同 compose 文件的多 service 允许部分成功（A 成功、B 失败互不影响）。

所有 worker 结束后，sweep 整个 `./data/temp/` 目录，然后释放 easy-deploy.lock。
...
**Step0. config_hash 比对**

- 计算当前 service 的 `package + deploy` 配置 hash，与 `current-versions.json` 中的 `config_hash` 比对
- `current-versions.json` 不存在时会初始化为空对象，按首次运行处理；如果文件已存在但不可读或不是合法 JSON，则跳过本轮，避免误判为版本为空后强制部署
- 缺失或不一致 → `force` 模式：package 脚本传入第二参 `force`，跳过 version/digest/blocked 的 skip 逻辑，强制拉取/下载并 deploy
- 一致 → 正常 package 流程

worker 计算当前配置 hash 后立即写入 `current-versions.json`，同时注册 EXIT trap：无论成功或失败，退出前按**当前配置**兜底回存 `config_hash`。
版本状态写入使用 `data/` 目录内的临时文件再替换 `current-versions.json`，避免跨文件系统 `mv` 造成非原子替换窗口。

**Step1. 执行 package 逻辑**

如果type是 generic，把 serviceName（及可选 `force`）传给 `package-generic.sh` 执行，阻塞等待返回值

如果type是 docker-container，把 serviceName（及可选 `force`）传给 `package-docker-container.sh` 执行，阻塞等待返回值

**Step2. 执行 deploy 逻辑**

首先，Step1得成功，失败的话就不执行了，此脚本报错结束

其次，Step1成功后，判断返回值如果是 `skip_deploy` 且非 force 模式，此脚本正常结束（不会进入 deploy，因此 frontend-dist 也不会触发 nginx reload）。force 模式下 package 不应返回 `skip_deploy`。

如果strategy是 frontend-dist，把 serviceName 和 package 步骤返回值 传给 `deploy-frontend-dist.sh` 执行

如果strategy是 docker-compose，把 serviceName 和 package 步骤返回的 digest 传给 `deploy-docker-compose.sh` 执行

如果strategy是 docker-run，把 serviceName 和 package 步骤返回值 传给 `deploy-docker-run.sh` 执行

**Step1 超时**：worker 用 `timeout` 包裹 package 脚本，上限为 `scripts.package-timeout-seconds`（默认 60s），限制整个 package 流程（含 generic 的 Gitea 查询/下载与 docker-container 的 pull），与 `package.type` 无关。
...
**package 阶段 hook**（在 `package-generic.sh` / `package-docker-container.sh` 内触发，worker 本身不触发 hook）：

| Hook | 何时触发 |
|------|----------|
| `on-package-start` | 查 Gitea / `docker pull` 之前 |
| `on-package-skip` | 正常模式且判定 skip 时（见下）；**force 模式不触发** |
| `on-package-success` | 本次将进入 deploy：有新 version/digest，或 **force 下同 version/digest** |
| `on-package-fail` | pull/下载/解析失败 |

`on-package-skip` 仅两种正常 skip：`version_tag` / digest 未变；docker-container 另含 digest 等于 `blocked_version_tag`。skip 时不进入 deploy，故无 deploy hook。

## 事件钩子（hooks）

配置与变量说明见 [config.doc.md](../config.doc.md#部署事件钩子hooks)。以下为与执行流程对齐的**代码实际触发点**（hook 命令失败不中断部署）。

### Agent 级

| Hook | 脚本 | 时机 |
|------|------|------|
| `on-agent-start` | `easy-deploy-agent.sh` | 启动 worker **之前**（在 `versions_ensure` 之前） |
| `on-agent-success` | 同上 | 全部 worker 成功且清空 `data/temp/` **之后** |
| `on-agent-fail` | 同上 | 存在 worker 失败且清空 temp **之后** |

### Deploy 级（按 strategy）

| strategy | 脚本 | `on-deploy-start` | `on-deploy-success` | `on-deploy-fail` |
|----------|------|-------------------|---------------------|------------------|
| `frontend-dist` | `deploy-frontend-dist.sh` | 解压前 | `versions_set` 且 nginx reload **成功**后 | 解压/部署失败时；**或** 文件已写入、`versions_set` 后 nginx reload 失败 |
| `docker-compose` | `deploy-docker-compose.sh` | flock / 改 yml **前** | `versions_set`、删旧镜像 **后** | **先回滚** yml + recreate → 再 hook → `versions_set_blocked` |
| `docker-run` | `deploy-docker-run.sh` | 第一个容器 `docker run` **前** | 全部容器稳定性通过、`versions_set` **后** | **先** hook → 再全量回滚 → `versions_set_blocked` |

deploy 阶段 `${hook_package_version_tag}`：frontend 为 Gitea version；docker 策略为 digest（`sha256:...`）。

## package-generic.sh

**此脚本执行必要的入参依次：`serviceName`、可选 `force`**

force 时跳过「version 与 current-versions 相同」的 skip，仍查 latest 并重新下载。
...
如果获取到了version，与 `current-versions.json` 比对：**相同且非 force** → `on-package-skip` → 返回 `skip_deploy`；**相同且 force**（配置变更）→ 仍下载 → `on-package-success`
...
force 时跳过 digest 与 `version_tag` / `blocked_version_tag` 相同的 skip，仍 pull 并输出 digest 供 deploy。
...
如果成功pull，与 `current-versions.json` 比对：**digest 与 `version_tag` 相同且非 force** → `on-package-skip` → `skip_deploy`；**digest 与 `blocked_version_tag` 相同且非 force** → `on-package-skip` → `skip_deploy`；**force**（配置变更）→ 跳过上述判断 → `on-package-success` 并输出 digest
...
**Hook**：`on-deploy-start`（解压前）→ 失败路径 `on-deploy-fail` → 成功：`versions_set` → nginx reload → `on-deploy-success`。若 reload 失败，此时 `version_tag` 已更新，仍会 `on-deploy-fail`。

## deploy-docker-compose.sh

**此脚本执行必要的入参依次：`serviceName`、`imageDigest`**

单 service 完整 deploy 流程（与 `deploy-docker-run.sh` 对齐：成功/失败/回滚/versions/blocked 均只作用于本 service）：

1. `on-deploy-start`
2. `flock` `${COMPOSE_FILE}.easy-deploy.lock`（同 compose 文件串行，不同文件互不阻塞）
3. 备份 yml → `${composeFile}.easy-deploy.bak`；`yq` 仅 patch 本 service 的 image
4. `docker compose up -d --no-deps --force-recreate <composeService>`（不 down 整栈、不重建 sibling）
5. `container_stability_check`（本 service 的 `started-check-seconds`；`-1` 跳过）
6. **成功**：`versions_set`（清除 blocked）、删旧 digest 镜像、`on-deploy-success`、touch `.deploy-executed`
7. **失败**（仅回滚本 service）：恢复 yml backup → `up --no-deps --force-recreate` 仅本 composeService → 删新 digest → **`on-deploy-fail`** → `versions_set_blocked`（仅本 service）
...
**失败回滚（原子语义）**：任一实例 run 失败或稳定性检查失败 → **`on-deploy-fail`** → 删除本 service 全部容器 → 若有旧 digest 则按 `containers` 顺序用旧 digest 重启全部实例 → 删除新 digest 镜像 → `versions_set_blocked` → 报错退出。

**成功**：`on-deploy-start`（循环前）→ 各容器 deploy + 稳定性检查 → `versions_set` → `on-deploy-success` → 删除旧 digest 镜像。
