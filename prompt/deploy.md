...
1. 读取 `./easy-deploy-config.yaml` 里所有 `services` 的 `name`，并行启动 `easy-deploy-worker.sh`
2. **等待全部 worker** 结束（失败计入 failures）
3. 清空 `./data/temp/`，释放锁，执行 agent 级 hook 与日志保留
...
7. **失败**（仅回滚本 service）：恢复 yml backup → `up --no-deps --force-recreate` 仅本 composeService → 删新 digest → `versions_set_blocked`（仅本 service）→ `on-deploy-fail`

Hook 均在 `deploy-docker-compose.sh` 内触发：`on-deploy-start` 在 flock 前；`on-deploy-success` / `on-deploy-fail` 在对应结果路径。
